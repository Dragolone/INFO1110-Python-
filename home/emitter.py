...
def check_direction(direciton):
    target_direciton = ["N", "E", "S", "W"]
    index = 0
    while index < len(target_direciton):
        if direciton == target_direciton[index]:
            return True
        else:
            index = index + 1
    return False
...
class Emitter:
    ...
    def set_pulse_sequence(self, frequency: int, direction: str) -> None:
        ...
        '''
        if frequency > 0 and check_direction(direction):
    ...
    def __str__(self) -> str:
        ...
        Info = f"{self.symbol}: {self.frequency}THz, {print_direc}"
        return Info
